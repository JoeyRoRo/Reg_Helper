...
class App(object):
        ...
        for arg in set(self._args_map.values()):
            if arg.name not in self._context:
                if arg.default:
                    ...
                    if arg.index:
                        self._context['index{}'.format(
                            arg.index)] = arg.default
                else:
    ...
    def _display_usage(self, exit=True):
        ...
        usage_str = ''
        if self._flags:
            usage_str += '[-{}]'.format(''.join([arg._short.strip('-')
                                                 for arg in self._flags]))
        if self._opts:
            usage_str += ' [{}]'.format(' '.join([' '.join([arg._short,
                                                            arg._name])
        ...
        if self._pos_args:
            usage_str += ' [{}]'.format(' '.join([arg._name
